    ...
    while running:
        ...
        for _ in range(sub_steps):
            ...
            if env.path_timer >= 0.01:
                env.path_timer = 0
                if env.current_wp is None:
                    # 목적지까지 장애물이 없어 바로 목적지를 추종하는 경우: 곡률을 최소화한 처음 기본 곡선
                    goal = env.target
                    env.bezier_path = make_bezier_path(env.boat_pos, env.boat_heading, goal, is_direct=True)
                else:
                    # 장애물 갭(웨이포인트) 통과 시: 부드럽게 돌아나가는 곡률 곡선
                    goal = env.current_wp["pos"]
                    env.bezier_path = make_bezier_path(env.boat_pos, env.boat_heading, goal, is_direct=False)
                ...
                if env.current_wp is not None and env.next_wp is not None:
                    ...
                    env.next_bezier_path = make_bezier_path(env.current_wp["pos"], next_start_head, env.next_wp["pos"], is_direct=False)
