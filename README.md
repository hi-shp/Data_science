...
라이다(LiDAR) 계측 데이터에 DBSCAN 군집화 기법을 적용하여 동적 장애물 사이 유효 간극(Free-space Gap)의 중심점을 추출하고, 3차 베지어 곡선(Cubic Bézier Curve) 및 순수추종(Pure Pursuit) 제어기를 결합하여 실시간 국소 경로를 생성·추종하는 2D 자율운항 시뮬레이션 시스템 연구개발 결과입니다.
...
- **연구 성과**: 무거운 전역 SLAM 시스템 없이도, 1차원 라이다 거리 프로파일에서 가항 영역 중심 좌표를 추출하고 3차 베지어 곡선으로 추종하는 기하학적 접근법을 통해, 기존 반사식 제어의 고질적 문제였던 선체 진동 및 정체 현상을 성공적으로 극복함.
- **실용적 의의**: 센서 탑재 및 컴퓨팅 파워가 제약되는 소형 수상 무인선 플랫폼에서, 경량 알고리즘만으로 SLAM 핵심 기능(자유 공간 인지 및 부드러운 국소 경로 생성)의 약 70% 수준을 실시간 구현할 수 있음을 실증함.
- **향후 과제**: 조류·풍랑 등 외란 환경에서의 조향 강건성 확보 및 간극 평가 가중치 파라미터의 환경 적응형 자동 튜닝 연구가 요구됨.
